...
\end{enumerate}
We implement this architecture in an open-source C++20 template library supporting arbitrary Dirichlet convolutions. We report empirical benchmarks scaling up to $X = 10^{16}$ (10 quadrillion), computing $M(10^{12}) = 62,366$ in \textbf{0.21 seconds}, $M(10^{14}) = -875,575$ in \textbf{8.53 seconds}, $M(10^{15}) = -3,216,373$ in \textbf{79.56 seconds}, and $M(10^{16}) = -3,195,437$ in \textbf{622.43 seconds} ($\sim 10.37$ minutes) on a consumer workstation with 15 parallel threads.
\end{abstract}
...
$10^{14}$ & $-875,575$ & $3,039,635,509,270,144,893,910,357,854$ & $-7,424,752$ & $3,204,941,750,802$ & \textbf{8.5297 s} \\
...
\begin{itemize}
    ...
    \item \textbf{Thread Pool:} 15 OpenMP worker threads operating under guided task distribution (\texttt{schedule(guided)}). Moderate oversubscription ($15/8 \approx 1.9\times$) empirically improved throughput by keeping all cores saturated during load-imbalanced doubling stages, yielding $\sim 20$--$40\%$ speedup over $8$ threads at $X \ge 10^8$.
\end{itemize}
...
Table \ref{tab:multirun} reports mean and standard deviation for Mertens evaluations across $3$ independent runs at $15$ OpenMP threads, providing error bounds for the timings in Table~\ref{tab:benchmarks}.
...
\midrule
$10^{10}$ & $0.016$ & $\pm 0.004$ \\
$10^{11}$ & $0.044$ & $\pm 0.008$ \\
$10^{12}$ & $0.220$ & $\pm 0.009$ \\
$10^{13}$ & $1.090$ & $\pm 0.126$ \\
$10^{14}$ & $6.066$ & $\pm 0.117$ \\
\bottomrule
\end{tabular}
\caption{Multi-run Mertens timing statistics with $15$ OpenMP threads ($3$ runs per scale).}
\label{tab:multirun}
\end{table}
