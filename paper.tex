...
$10^{15}$ & $-3,216,373$ & $303,963,550,927,013,509,478,708,835,152$ & $-29,445,104$ & $29,844,570,422,669^\dagger$ & \textbf{79.5612 s} \\
$10^{16}$ & $-3,195,437$ & $30,396,355,092,701,332,166,351,822,199,504$ & $-97,617,938$ & $279,238,341,033,925^\dagger$ & \textbf{622.4313 s} \\
\bottomrule
\end{tabular}%
}
\caption{Multi-scale evaluation across arithmetic functions using 15 OpenMP threads, 2-part vectorization, and hybrid linear pre-sieving ($u$-cutoff). Results up to $10^{16}$ match established mathematical literature \cite{kotnik2004order, kuznetsov2011computing, hurst2026practical}. ${}^\dagger$Values from published tables, not computed by our engine.}
...
The coordinate bijection $\tau$ enables purely sequential memory access within each doubling stage. For dense-prefix queries ($q \le S$), the index $\tau(q) = q - 1$ produces a linear stride-1 traversal of the array, achieving optimal spatial locality. For sparse-suffix queries ($q > S$), the index $\tau(q) = N - \floor{X/q}$ accesses the upper portion of the array in monotonically increasing order as $q$ decreases through the inner loop. Combined with the pre-sieve buffer (which fits entirely in L2/L3 cache for $u \le 10^8$), the vast majority of inner-loop memory accesses are cache-resident, avoiding the random-access patterns inherent in hash-table-based implementations.
...
Table \ref{tab:scaling} reports wall-clock times for $M(10^{12})$ across thread counts, averaged over $3$ independent runs. Peak performance is achieved at $8$ threads (matching the physical core count), with moderate degradation under oversubscription at $15$ threads.
