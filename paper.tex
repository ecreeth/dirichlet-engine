...
\author{
    ...
    \texttt{\small \href{https://github.com/oss/dirichlet-engine}{https://github.com/oss/dirichlet-engine}}
}
...
\end{enumerate}
We implement this architecture in an open-source C++20 template library supporting arbitrary Dirichlet convolutions. We report empirical benchmarks scaling up to $X = 10^{15}$ (1 quadrillion), computing $M(10^{12}) = 62,366$ in \textbf{0.96 seconds}, $M(10^{14}) = -875,575$ in \textbf{29.80 seconds}, and $M(10^{15}) = -3,216,373$ in \textbf{160.18 seconds} (2.67 minutes) on consumer hardware with 15 parallel threads.
\end{abstract}
...
\toprule
\textbf{Target $X$} & \textbf{Mertens $M(X)$} & \textbf{Totient $\Phi(X)$} & \textbf{Liouville $L(X)$} & \textbf{$\pi(X)$} & \textbf{Runtime (s)} \\
\midrule
$10^7$ & $1,037$ & $30,396,356,427,242$ & $-842$ & $664,579$ & \textbf{0.0012 s} \\
$10^8$ & $1,928$ & $3,039,635,538,981,878$ & $-530$ & $5,761,455$ & \textbf{0.0054 s} \\
$10^9$ & $-222$ & $303,963,551,391,249,150$ & $-14$ & $50,847,534$ & \textbf{0.0150 s} \\
$10^{10}$ & $-33,722$ & $30,396,355,092,886,216,366$ & $-116,026$ & $455,052,511$ & \textbf{0.0503 s} \\
$10^{11}$ & $-87,856$ & $3,039,635,510,883,674,842,482$ & $-447,214$ & $4,118,054,813$ & \textbf{0.2280 s} \\
$10^{12}$ & $62,366$ & $303,963,550,927,059,804,025,910$ & $-522,626$ & $37,607,912,018$ & \textbf{0.9646 s} \\
$10^{13}$ & $599,582$ & --- & --- & --- & \textbf{5.2735 s} \\
$10^{14}$ & $-875,575$ & --- & --- & --- & \textbf{29.7968 s} \\
$10^{15}$ & $-3,216,373$ & --- & --- & --- & \textbf{160.1818 s} \\
\bottomrule
\end{tabular}
\caption{Multi-scale evaluation across arithmetic functions using 15 OpenMP threads and 2-part vectorization. Results up to $10^{15}$ match established mathematical literature \cite{kotnik2004order, kuznetsov2011computing}.}
\label{tab:benchmarks}
\end{table}

\subsection{Speedup and Scaling Discussion}
At $X = 10^{13}$, the parallel doubling schedule computes $M(10^{13}) = 599,582$ in \textbf{5.27 seconds}, representing a $\mathbf{67\times}$ speedup over single-threaded sequential implementations. Scaling to $X = 10^{15}$ (1 quadrillion), the state space $|\V(X)| \approx 6.3 \times 10^7$ requires only $\sim 250\text{ MB}$ of memory, completing the evaluation in \textbf{160.18 seconds} (2.67 minutes).
...
Future work includes extending the doubling framework to $\Oh(X^{2/3})$ combinatorial sieves and developing GPU/MPI implementations for computations at $X \ge 10^{18}$.
