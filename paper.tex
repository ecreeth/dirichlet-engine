...
\begin{itemize}
    ...
    \item The \textbf{Prime counting function} $\pi(X) = \sum_{p \le X} 1$ \cite{lucy2011algorithm}.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Hyperbolic Dynamic Programming (Lucy DP / Min-25):} For general Dirichlet convolutions $h = f * g$, grouping lattice points under integer hyperbolas yields an exact sublinear recurrence over the quotient space $\V(X) = \{\floor{X/i} : 1 \le i \le X\}$ in $\Oh(X^{3/4})$ operations and $\Oh(\sqrt{X})$ memory \cite{lucy2011algorithm, min25sieve}.
\end{enumerate}
...
\midrule
$10^{12}$ & $62{,}366$   & $0.196$ s & $1.185$ s  & $3.283$ s \\
$10^{14}$ & $-875{,}575$ & $5.608$ s & $23.899$ s & $34.212$ s \\
$10^{15}$ & $-3{,}216{,}373$ & $51.943$ s & $264.207$ s & $143.224$ s \\
$10^{16}$ & $-3{,}195{,}437$ & $511.980$ s & --- & $692.897$ s \\
\bottomrule
\end{tabular}
