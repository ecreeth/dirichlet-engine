...
\author{
    ...
    \texttt{\small \href{https://github.com/oss/eks}{https://github.com/oss/eks}}
}
...
\end{enumerate}
We implement this architecture in an open-source C++20 template library supporting arbitrary Dirichlet convolutions. We report empirical benchmarks scaling to $X = 10^{14}$ (100 trillion), computing $M(10^{12}) = 62,366$ in \textbf{0.96 seconds} and $M(10^{14}) = -875,575$ in \textbf{29.80 seconds} on consumer hardware with 15 parallel threads.
\end{abstract}
...
\bottomrule
\end{tabular}
\caption{Multi-scale evaluation across arithmetic functions using 15 OpenMP threads and 2-part vectorization. Results up to $10^{14}$ match established mathematical literature \cite{kotnik2004order, kuznetsov2011computing}.}
\label{tab:benchmarks}
\end{table}

\subsection{Speedup and Scaling Discussion}
At $X = 10^{13}$, the parallel doubling schedule computes $M(10^{13}) = 599,582$ in \textbf{5.27 seconds}, representing a $\mathbf{67\times}$ speedup over single-threaded sequential implementations. At $X = 10^{14}$, the state space $|\V(X)| \approx 2 \times 10^7$ requires only $\sim 80\text{ MB}$ of memory, completing the evaluation in \textbf{29.80 seconds}.
