...
Table \ref{tab:scaling} and Figure \ref{fig:scaling_plot} report wall-clock times and speedups for $M(10^{12})$ across thread counts, averaged over $3$ independent runs. Peak performance is achieved at $8$ threads (matching the physical core count of the Apple M1), achieving a $3.00\times$ speedup, with minor efficiency drop under oversubscription at $15$ threads due to task scheduler overhead.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[scale=0.88]
    % Coordinate axes
    \draw[->, thick, gray!80] (0,0) -- (16,0) node[right, black] {\small \textbf{Threads ($T$)}};
    \draw[->, thick, gray!80] (0,0) -- (0,5.5) node[above, black] {\small \textbf{Speedup ($\mathcal{S}$)}};
    
    % Grid lines
    \foreach \y in {1, 2, 3, 4, 5} {
        \draw[dashed, gray!30] (0,\y) -- (15.5,\y);
        \node[left, scale=0.8] at (0,\y) {$\y\times$};
    }
    \foreach \x in {1, 2, 4, 8, 15} {
    ...
    % Points: (1, 1.00), (2, 1.83), (4, 2.81), (8, 3.00), (15, 2.56)
    ...
    \node[above=3pt, blue!90!black, scale=0.75] at (2, 1.83) {$1.83\times$};
    \node[above=3pt, blue!90!black, scale=0.75] at (4, 2.81) {$2.81\times$};
    \node[above=3pt, blue!90!black, scale=0.75] at (8, 3.00) {\textbf{3.00}$\times$ (peak)};
    \node[below=4pt, blue!90!black, scale=0.75] at (15, 2.56) {$2.56\times$};
    ...
    \node[orange!70!black, scale=0.75, anchor=north east] at (15.3, 5.0) {Oversubscribed ($T > 8$)};
\end{tikzpicture}
\caption{Thread scaling for $M(10^{12})$ on Apple M1 (8 cores). Speedup scales near-linearly up to 4 cores, reaches peak at 8 physical cores ($3.00\times$), and exhibits scheduler contention when oversubscribed.}
...
\midrule
$1$  & $0.563$ & $1.00\times$ & $100\%$ \\
$2$  & $0.307$ & $1.83\times$ & $92\%$ \\
$4$  & $0.200$ & $2.81\times$ & $70\%$ \\
$8$  & $0.188$ & $3.00\times$ & $37\%$ \\
$15$ & $0.220$ & $2.56\times$ & $17\%$ \\
\bottomrule
\end{tabular}
